...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname exercise_42_5_27) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;#lang racket
(require 2htdp/image)
...
; WorldState -> WorldState
; translate the worldstate so that
; it represent the right most edge
; given: 20 expect: 37.5
(define (RIGHT_EDGE cw)
  (+ cw (* WHEEL_RADIUS 3.5))
  )
; WorldState -> Image
; place CAR on BACKGROUND with respect to WorldState
(define (render cw)
  (place-image CAR (RIGHT_EDGE cw) Y_CAR BACKGROUND))
; WorldState -> boolean
; determine if the WorldState is greater
; than 740, stop
(define (WORLD_COND ws)
  (if (< (RIGHT_EDGE ws) WORLD_LENGTH) #false #true)
  )
; WorldState -> WorldState
