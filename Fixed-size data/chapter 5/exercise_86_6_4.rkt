...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname exercise_86_6_4) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(check-expect (check-length (make-editor "hello" "world")) #false)
(check-expect (check-length
(make-editor "helloworldhelloworldhelloworldhelloworldw" "")) #true)
(define (check-length ed)
  (>= (image-width (insert-curser ed)) 195)
  )
; Editor -> Editor
; condition 1: if the character counts is below 35,
;              add the aditional charcter
; condition 2: if the character count is above or equal to 35,
;              return input editor
(define (append-str ed ke)
  (cond
    [(check-length ed) ed]
    [else (move-add ed ke)]
    )
...
(define editor-check-editor (make-editor "hellohellohellohellohellohellohellohellohellohellohellohellohello" "worldd"))
...
(check-expect (edit te "\t") te)
(check-expect (edit te "nonsense") te)
(check-expect (edit editor-check-editor "q") editor-check-editor)
(define (edit ed ke)
  (cond
    [(string=? ke "\b") (move-back ed)]
    [(> (string-length ke) 1) (move-cursor ed ke)]
    [(or (string=? ke "\t") (string=? ke "\r")) ed]
    [(= (string-length ke) 1) (append-str ed ke)]
    )
  )
(define (main holder)
