...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname exercise_323_7_31) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(check-expect (search-bt 77 tree-1) 'g)
(check-expect (search-bt 100 tree-1) #f)
(define (search-bt n bt)
  (cond
    [(no-info? bt) #false]
    [(= (node-ssn bt) n) (node-name bt)]
    [else ((lambda (x y) (if (boolean? x) y x)) (search-bt n (node-left bt)) (search-bt n (node-right bt)))]))
