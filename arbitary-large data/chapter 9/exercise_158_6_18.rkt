...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname exercise_158_6_18) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
; auxiliary function
; Number -> boolean
; determine if the first shot has reached the end
(check-expect (shot? 0) #t)
(check-expect (shot? 80) #f)
(define (shot? w)
  (if (<= w 0) #t #f))
; ShotWorld -> ShotWorld 
; moves each shot up by one pixel
(check-expect (tock ex) (cons 19 (cons 29 (cons 39 '()))))
(check-expect (tock (cons 0 ex)) (cons 19 (cons 29 (cons 39 '()))))
(define (tock w)
  (cond
    [(empty? w) '()]
    [else
